# 02. BCE·계약·RED·통합 보고서 - Prompting

## 1. 사용 프롬프트

### 1.1 BCE·계약·테스트·통합 설계 요청

```text
Dual-Track(UI/경계 vs Domain) + BCE 관점 설계 전문가로 행동하라.
대상: @c:\dev\UnitConverter_A02\
...
```text
이번세션에서 진행한 내용을 report 폴더를 만들어서 02_BCE·계약·RED·통합_보고서,md 파일로 내보내주고,
Prompting 폴더를 만든 후 프롬프트를 포함해서 02_BCE·계약·RED·통합_보고서-Prompting.md 파일로 내보내줘
```

파일 확장자는 문맥상 Markdown 파일로 해석하여 `02_BCE·계약·RED·통합_보고서.md`로 생성했다.

## 2. 프롬프트 해석 기준

이번 세션의 모든 산출물은 다음 제약을 따른다.

- 구현 코드는 작성하지 않는다.
- JUnit 5 테스트 코드도 작성하지 않고 테스트 제목과 보호 invariant만 정의한다.
- BCE 책임을 Boundary, Control, Entity, Data로 분리한다.
- Domain 규칙과 Boundary 계약을 별도 트랙으로 고정한다.
- 모든 규칙은 테스트로 검증 가능한 문장으로 작성한다.
- README와 PRD에 존재하는 `meter`, `feet`, `yard`, meter 허브, 입력 검증, 출력 보존, 설정 실패, 동적 등록 요구를 기준으로 한다.
...
- JSON, CSV, 표 출력은 동일한 Domain 결과를 다른 형식으로 표현해야 한다.
...
이번 프롬프트는 구현 금지, 설계 범위, 테스트 목록, 통합 검증을 명확히 분리했다. 따라서 산출물이 코드로 흐르지 않고 계약과 검증 기준으로 유지되었다.

다음 세션에서 더 정밀하게 만들려면 아래 조건을 추가하면 좋다.

- 변환 수치 허용 오차를 `±0.000001`처럼 고정한다.
- 출력 반올림 정책을 `HALF_UP`, 소수점 1자리처럼 고정한다.
- 오류 detail 문구까지 exact match할지 prefix match할지 지정한다.
- 동적 등록 입력 형식 `register:<unit>:<metersPerUnit>`을 별도 Boundary 계약으로 분리한다.
- JSON/YAML 중 v1에서 먼저 구현할 형식을 하나로 제한한다.

## 14. 결론

이번 세션의 핵심 결과는 `UnitConverter_A02`의 구현 이전 안전망을 만든 것이다. Domain은 단위와 비율을 보호하고, Boundary는 입력·출력·오류 계약을 보호하며, Data는 설정 로드 실패를 분리한다. RED 단계 테스트 제목은 이후 구현 순서를 결정하는 체크리스트로 사용할 수 있다.
